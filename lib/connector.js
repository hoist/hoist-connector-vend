...
VendConnector.prototype.refreshToken = function () {
  if (this.authSettings) {
    if (this.authSettings.token.expires < Date.now()) {
      var body = {
      ...
      return this.requestPromiseHelper(options)
        ...
        .then(function (response) {
          if (response.body) {
            return this.authSettings.set('token', response.body);
...
VendConnector.prototype.receiveBounce = function (bounce) {
  if (bounce.get('query')) {
    var query = bounce.get('query');
    return bounce.set('code', query.code)
      .then(function () {
        return bounce.set('domainPrefix', query.domain_prefix);
      })
      .bind(this)
      .then(function () {
      ...
      .then(function (response) {
        return bounce.set('token', response.body);
      })
      .then(bounce.done);
  } else {
...
VendConnector.prototype.requestAccessToken = function (bounce) {
  ...
  var options = {
    method: 'POST',
    uri: 'https://' + this.authSettings.get('domain_prefix') + apiUrl + '/1.0/token',
    body: body,
    json: true,
    resolveWithFullResponse: true
  };
