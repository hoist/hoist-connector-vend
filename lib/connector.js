...
VendConnector.prototype.subscribe = function (subscription, authorization) {
  return this.authorize(authorization).bind(this).then(function () {
    return BBPromise.settle(_.map(subscription.endpoints, _.bind(function (endpoint) {
      //check if already created for token
      var endpointMeta = subscription.get(endpoint);
      if (endpointMeta && endpointMeta[authorization._token._id]) {
        return;
      }
      return this.post('/webhooks', {
        type: endpoint,
        active: true,
        url: 'https://endpoint.hoi.io/webhooks/' + subscription._id + '/' + endpoint + '/' + authorization._token._id
      }).then(function (res) {
        //update meta on subscription
        // subscription.meta[endpoint][authorization._token._id].id = res.id;
        if (res.id) {
          var meta = {};
          meta[authorization._token._id] = {
            id: res.id
          };
          return subscription.set(endpoint, meta);
        }
      });
    }, this)));
  });
...
VendConnector.prototype.unsubscribe = function (subscription, authorization, endpoint) {
  return this.authorize(authorization).bind(this).then(function () {
    var endpointMeta = subscription.get(endpoint);
    console.log(endpointMeta);
    console.log(authorization);
    if (!endpointMeta || !endpointMeta[authorization._token._id] || !endpointMeta[authorization._token._id].id) {
      return;
    }
    return this.delete('/webhooks/' + endpointMeta[authorization._token._id].id)
      .then(function (res) {
        if (res.status === 'success') {
          return subscription.delete([endpoint, authorization._token._id]);
        }
      });
  });
};
