...
VendConnector.prototype.subscribe = function (subscription, bounceToken) {
  return this.authorize(bounceToken).bind(this).then(function () {
    return BBPromise.settle(_.map(subscription.endpoints, _.bind(function (endpoint) {
      ...
      if (endpointMeta && endpointMeta[bounceToken.get('_id')]) {
        return;
      }
      return this.post('/webhooks', {
        type: endpoint,
        active: true,
        url: 'https://endpoint.hoi.io/webhooks/' + subscription._id + '/' + endpoint + '/' + bounceToken.get('_id')
      }).then(function (res) {
        //update meta on subscription
        // subscription.meta[endpoint][bounceToken.get('_id')].id = res.id;
        if (res.id) {
          var meta = {};
          meta[bounceToken.get('_id')] = {
            id: res.id
          };
